...
import { LogiFinderLoading } from "../components/LogiFinderLoading";
...
function GlobalUserNameFix() {
  useEffect(() => {
    const normalize = () => {
      ...
      const replacements: Array<[RegExp, string]> = [[/\bMarcos\b/gi, "Alexsandro"], [/\bMarcor\b/gi, "Alexsandro"], [/\bMarcoso\b/gi, "Alexsandro"]];
      let node: Node | null;
...
function TrackingHomeButton() {
  ...
  return (
    <Link to="/" aria-label="Voltar para o início" className="fixed left-5 top-5 z-[100] inline-flex items-center gap-2 rounded-xl border border-primary/50 bg-background/95 px-4 py-2.5 text-sm font-semibold text-primary shadow-[0_0_24px_-8px_var(--color-primary)] backdrop-blur-xl transition-all hover:bg-primary hover:text-primary-foreground">
