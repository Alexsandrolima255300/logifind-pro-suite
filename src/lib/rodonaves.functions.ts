...
export const getRodonavesStatus = createServerFn({ method: "GET" }).handler(async () => {
...
export const connectRodonaves = createServerFn({ method: "POST" }).handler(async () => {
...
export const disconnectRodonaves = createServerFn({ method: "POST" }).handler(async () => {
  const { clearToken } = await import("./rodonaves.server");
  clearToken();
  return { ok: true as const };
});

export const discoverRodonavesEndpoints = createServerFn({ method: "GET" }).handler(async () => {
