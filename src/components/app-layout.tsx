...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="lf-shell min-h-screen w-full">
      {/* Desktop: zona invisível de 24px na borda esquerda + menu deslizante. */}
      ...
      <aside
        ...
        className={cn(
          ...
          !mobileOpen && "-translate-x-[calc(100%-24px)] md:block",
      ...
      >
        ...
        {/* A lista agora possui rolagem própria: todas as abas ficam acessíveis pela bolinha do mouse. */}
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pt-5 pb-4 [scrollbar-width:thin] [scrollbar-color:rgba(255,255,255,.18)_transparent]">
          ...
          <nav className="space-y-1">
            {nav.map((item) => {
              ...
              return <Link key={item.to} to={item.to} onClick={() => setMobileOpen(false)} className={cn("lf-nav-item", active && "active")}>
        ...
        <div className="shrink-0 border-t border-white/7 p-4">
          <div className="lf-user-card">
            ...
            <div className="min-w-0"><p className="truncate text-sm font-semibold">Alexsandro</p><p className="text-[10px] text-muted-foreground">Administrador</p></div>
          </div>
        </div>

        {/* Seta sempre acessível para abrir/fechar manualmente. */}
        <button
          type="button"
          aria-label={sidebarOpen ? "Esconder menu lateral" : "Abrir menu lateral"}
          title={sidebarOpen ? "Esconder menu" : "Abrir menu"}
          onClick={() => setSidebarOpen((open) => !open)}
          className={cn(
            "absolute -right-4 top-1/2 hidden h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-background/95 text-muted-foreground shadow-xl transition-all hover:border-primary/40 hover:text-primary md:flex",
